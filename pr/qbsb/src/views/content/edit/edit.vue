<template>
  <div id="edit-edit" v-loading.lock="loading">
    <div class="tab">
      <div class="content">
            ...
            <div class="table">
              <el-table :data="article_list_2" style="width: 100%">
                ...
                <el-table-column align="center" width="220px" label="操作">
                  <template slot-scope="scope">
                    <el-button type="text" v-if="scope.row.status == 3" @click="clickUpdateToAudit(scope.row.id)">送审</el-button>
                    <el-button type="text" class="danger-color" v-if="scope.row.status == 3" @click="clickSendBack(scope.row.id)">打回</el-button>
                    <el-button type="text" class="wran-color" v-if="scope.row.status == 3 && scope.row.type == 1" @click="routerTo('/layout/edit-edite-article?vfrom=2&id='+ scope.row.id)">修改</el-button>
                    <el-button type="text" class="wran-color" v-if="scope.row.status == 3 && scope.row.type == 6" @click="routerTo('/layout/edit-edite-link?vfrom=2&id='+ scope.row.id)">修改</el-button>
              ...
              <el-table
                ...
                <el-table-column align="center" width="220px" label="操作">
                  <template slot-scope="scope">
                    <el-button type="text" v-if="scope.row.status == 3" @click="clickUpdateToAudit(scope.row.id)">送审</el-button>
                    <el-button type="text" class="danger-color" v-if="scope.row.status == 3" @click="clickSendBack(scope.row.id)">打回</el-button>
                    <el-button type="text" class="wran-color" v-if="scope.row.status == 3" @click="routerTo('/layout/edit-edite-video?vfrom=2&id='+ scope.row.id)">修改</el-button>
                    <el-button type="text" @click="openOperationLog(scope.row.cid)">操作记录</el-button>
                  </template>
                </el-table-column>
              </el-table>
    ...
    <!-- <PreviewArticle :articleId="previewArticleId" :apiUrl="'/context/approve/detail'" @closePreviewArticle="closePreviewArticle"></PreviewArticle> -->
        ...
        self.$confirm('建议送审前再检查一遍，确定送审？', '送审提醒', {
          ...
          callback: function (action) {
            if (action == 'confirm') {
              self.loading=true;
              self.$ajax({
                url:'/context/approve/editor/updateStatus',
                data:{
                  id:id,
                  status:'4'
                },
