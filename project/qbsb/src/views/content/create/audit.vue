<template>
  <div id="content-audit" v-loading.lock="loading">
    <div class="tab">
      <div class="content">
              ...
              <div class="search">
                <el-form :inline="true" :model="article_search_form_2">
                  ...
                  <el-form-item label="父频道">
                    <el-select v-model="article_search_form_2.channelPid" placeholder="父频道" @change="getChildChannel">
                      <el-option label="全部" value=""></el-option>
                      <el-option v-for="item in newsParentChannelList" :label="item.name" :key="item.id" :value="item.id"></el-option>
                    </el-select>
                  </el-form-item>
                  <el-form-item label="子频道">
                    <el-select v-model="article_search_form_2.channelId" placeholder="子频道">
                      <el-option label="全部" value=""></el-option>
                      <el-option v-for="item in newsChildChannelList" :label="item.name" :key="item.id" :value="item.id"></el-option>
                    </el-select>
                  </el-form-item>
                  <el-form-item label="状态">
                    <el-select v-model="article_search_form_2.status" placeholder="状态">
                      <el-option v-for="item in searchStatusList" :key="item.status" :label="item.name" :value="item.status"></el-option>
                    </el-select>
                  </el-form-item>
                  <el-form-item>
            ...
            <div class="table">
              <el-table :data="article_list_2" style="width: 100%">
                ...
                <el-table-column align="center" prop="label" label="标签">
                  <template slot-scope="scope">
                    <a href="javascript:void(0)" @click="viewLabels">查看</a>  
                  </template>  
                </el-table-column>
                ...
                <el-table-column align="center" width="220px" label="操作">
                  <template slot-scope="scope">
                    <el-button type="text" v-if="scope.row.status == 1" @click="clickUpdateToAudit(scope.row.id)">送审</el-button>
                    <el-button type="text" class="danger-color" v-if="scope.row.status == 1" @click="clickSendBack(scope.row.id)">打回</el-button>
                    <el-button type="text" class="wran-color" v-if="scope.row.status == 1" @click="routerTo('/layout/create-create?vfrom=1&type='+scope.row.type+'&id='+ scope.row.id)">修改</el-button>
              ...
              <el-table
                ...
                <el-table-column align="center" width="220px" label="操作">
                  <template slot-scope="scope">
                    <el-button type="text" v-if="scope.row.status == 1" @click="clickUpdateToAudit(scope.row.id)">送审</el-button>
                    <el-button type="text" class="danger-color" v-if="scope.row.status == 1" @click="clickSendBack(scope.row.id)">打回</el-button>
                    <el-button type="text" class="wran-color" v-if="scope.row.status == 1" @click="routerTo('/layout/create-create?vfrom=1&type=3&id='+ scope.row.id)">修改</el-button>
                    <el-button type="text" @click="openOperationLog(scope.row.cid)">操作记录</el-button>
                  </template>
                </el-table-column>
              </el-table>
    ...
    <previewPop v-if="previewArticleId" :articleId="previewArticleId" @closePreviewArticle="closePreviewArticle"></previewPop>
    <OperationLog :articleId="operationLogId" @closeOperationLog="closeOperationLog"></OperationLog>
    <label-detail v-if="isShowLabelDetail" @fClose="isShowLabelDetail=false" :isShow="isShowLabelDetail"></label-detail>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    created() {
      if (this.$route.query.type) this.sub_tab_index_2 = (this.$route.query.type==3?'3':'1');
      this.init();
    },
    methods: {
      queryList () {
      ...
      getArticleListData2:function () {
        ...
        if(self.article_search_form_2.timeScope && self.article_search_form_2.timeScope[0]){
      ...
      getVideoListData2:function () {
        var self=this;
        var startTime='',endTime='';
        if(self.video_search_form_2.timeScope && self.video_search_form_2.timeScope[0]){
