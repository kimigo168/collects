<template>
  <div id="Create" v-loading="loading">
    <header class="layout-header">
      ...
      <button class="search-btn" @click="clickWrite()" v-if="vType!='3'">智能写作</button>
    </header>
      ...
      <div class="carousel-wrap" v-if="articleList.length>0 && !noArticle" :class="{'show-full':isShowFull,'no-full':!isShowFull}">
        ...
        <el-carousel :interval="4000" arrow="always" :autoplay="false" @change="carouselChange" type="card" height="650px">
          <el-carousel-item v-for="(item, index) in articleList" :key="index">
            <div class="item">
              <div class="scroll write-scroll" @click="selectArticle(item, index)">
...
export default {
  ...
  methods: {
    ...
    clickWrite () {
      ...
      _czc.push(['_trackEvent', this.vType === '1' ? '热点底稿' : '提纲底稿', '点击智能创作', this.articleTitle])
      if (this.vType === '3') {
        //汽车写作隐藏
        return
        ...
        // 逐字写作过程
          ...
          setTimeout(() => {
            this.writeByWordTimer[idx] = setInterval(() => {
              ...
              // })
              m++
              if (m >= firstContent.length || m === 150) {
                obj.summray = initContent
        ...
        if (this.isShowAnimate) {
          for (let i = 0; i < this.articleList.length; i++) {
            if (i === 0) {
              ...
              write(this.articleList[i], 11000, 0, i)
            }
          }
        }
    ...
    selectArticle (item, index) {
      ...
      // eslint-disable-next-line
      _czc.push(['_trackEvent', this.vType === '1' ? '热点底稿' : '提纲底稿', '编辑底稿', `第${index + 1}篇`])
  ...
  created () {
    if (sessionStorage.getItem('keyword')) {
      this.articleTitle = JSON.parse(sessionStorage.getItem('keyword'))
    }
    this.tgType = this.$route.query.name // 提纲写作的模板类型
  },
  mounted () {
    this.vType = sessionStorage.getItem('writeType')
    var dataSource
    try {
      dataSource = JSON.parse(sessionStorage.getItem('dataSource'))
    } catch (error) {
      dataSource = ''
    }
    if (dataSource && dataSource !== '') { // 写出内容
      let list = []
      if (this.vType === '2') {
        let html = dataSource.split('\n\n')
        let str = ``
        for (let it of html) {
          str += `<p>${it}</p>`
        }
        list.push({
          contentHtml: str,
          summray: dataSource.replace(/\n{2}/g, '\n\n')
        })
      }else if (this.vType === '3') {
        for (let i = 0; i < dataSource.length; i++) {
          list.push({
            contentHtml: dataSource[i],
            summray: dataSource[i]
          })
        }
      }else {
        list = dataSource
